    ...
    <div class="space-y-6 p-6">
        ...
        <form class="overflow-hidden rounded-xl border bg-background shadow-sm" @submit.prevent="submit">
            ...
            <div class="grid gap-5 px-6 py-6 sm:grid-cols-2">
                ...
                <!-- Progress -->
                <div>
                    <label for="progress" class="mb-1.5 flex items-center justify-between text-sm font-medium">
                        <span class="flex items-center gap-1.5">
                            <Gauge class="h-4 w-4 text-muted-foreground" />
                            Progress
                        </span>
                        <span class="text-muted-foreground">{{ form.progres }}%</span>
                    </label>
                    <input id="progress" v-model.number="form.progres" type="range" min="0" max="100" step="5"
                        class="w-full accent-primary" />
                    <p v-if="form.errors.progres" class="mt-1.5 text-xs text-rose-600 dark:text-rose-400">
                        {{ form.errors.progres }}
                    </p>
                </div>
